# Mathematics---For---Machine---Learning

Mathematics for Machine Learning Specialization coursework and projects

# Mathematics for Machine Learning Specialization

🔢 **Imperial College London × Coursera** | Coursework & Projects Repository


---

## 📋 Course Progress

| # | Course | Status | Key Insight |
|---|--------|--------|-------------|
| 1 | Mathematics for Machine Learning: Linear Algebra | 🔄️ 99% | - |
| 2 | Mathematics for Machine Learning: Multivariate Calculus | 🔄 Not Started | — |
| 3 | Mathematics for Machine Learning: PCA | 🔄 Not Started | — |

---

## 📚 What This Specialization Covers

- **Linear Algebra** — Vectors, matrices, eigenvalues, eigenvectors, and the Gram-Schmidt process
- **Multivariate Calculus** — Partial derivatives, gradient descent, optimization, and backpropagation math
- **PCA (Principal Component Analysis)** — Dimensionality reduction, covariance matrices, and projections

---

## 🛠️ Tools & Languages

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat&logo=numpy&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=flat&logo=jupyter&logoColor=white)

---



## 🔗 Specialization Link

[Mathematics for Machine Learning – Coursera](https://www.coursera.org/specializations/mathematics-machine-learning)

---

*Offered by Imperial College London*
